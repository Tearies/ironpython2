...
a signed integer.")]
        public static int crc32([BytesConversion]IList<byte> data, long baseValue=0L)
        {
            if(baseValue < int.MinValue || baseValue > uint.MaxValue)
                throw new ArgumentOutOfRangeException(ResourceManager.Default.GetResource("baseValue", "baseValue"));
...
Optional arg wbits is the window buffer size.")]
        ...
        internal static Exception zlib_error(ZStream zst, int err, string msg)
        {
            ...
            if(zmsg == null)
                return MakeError(string.Format(ResourceManager.Default.GetResource("error01", "Error {0} {1}"), err, msg));
            else
                return MakeError(string.Format(ResourceManager.Default.GetResource("error012", "Error {0} {1}: {2}"), err, msg, zmsg));
        }
