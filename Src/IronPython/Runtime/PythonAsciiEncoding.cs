...
namespace IronPython.Runtime {
    ...
    class NonStrictEncoderFallbackBuffer : EncoderFallbackBuffer {
        ...
        public override bool Fallback(char charUnknownHigh, char charUnknownLow, int index) {
            throw PythonOps.UnicodeEncodeError(ResourceManager.Default.GetResource("asciicodeccantencodecharacteru0x104xinposition1ordinalnotinrange128", "'ascii' codec can't encode character '\\u{0:X}{1:04X}' in position {1}: ordinal not in range(128)"), (int)charUnknownHigh, (int)charUnknownLow, index);
        }

        public override bool Fallback(char charUnknown, int index) {
            if (charUnknown > 0xff) {
                throw PythonOps.UnicodeEncodeError(ResourceManager.Default.GetResource("asciicodeccantencodecharacteru0xinposition1ordinalnotinrange128", "'ascii' codec can't encode character '\\u{0:X}' in position {1}: ordinal not in range(128)"), (int)charUnknown, index);
            }
