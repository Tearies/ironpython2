    ...
    public class ReversedEnumerator : IEnumerator {
        ...
        public static object __new__(CodeContext context, PythonType type, object o) {
            ...
            if(!pt.TryResolveSlot(context, "__getitem__", out getitem) ||
                ...
                throw PythonOps.TypeError(ResourceManager.Default.GetResource("argumenttoreversedmustbeasequence", "argument to reversed() must be a sequence"));
            }

            int length;
            if (!DynamicHelpers.GetPythonType(o).TryGetLength(context, o, out length)) {
                throw PythonOps.TypeError(ResourceManager.Default.GetResource("objectoftype0hasnolen", "object of type '{0}' has no len()"), DynamicHelpers.GetPythonType(o).Name);
            }
